...
def main(workingDir, title):
    ...
    #this is to return area
    # returnField = 'ALAND'
    # areavalueList = arcpy.da.SearchCursor(shapeSpecPath, returnField)
    # # print areavalueList
    #
    # returnField = 'STATEFP'
    # statevalueList = arcpy.da.SearchCursor(shapeSpecPath, returnField)
    #
    # returnField = 'COUNTYFP'
    # countyvalueList = arcpy.da.SearchCursor(shapeSpecPath, returnField)
    ...
    # for couty in CountyResList:  #there may jast one file here.
    #     #here is to get the outputpath accroding to clu
    #     AggLevel = "couty"
    #     #TODO:get the outputPath according to clu
    #     outputPath = CLU + ..
    #
    #     tempList = outputPath.split("\\")
    #     print tempList
    #     State = tempList[-3]
    #     StateLowerCase = State
    #     State = State.upper()
    ...
    #
    #     #this is to read the area attribtutes in the shape file
    #     readPath = workingDir + "CLU_US\\"                                  #this is to get the sub readPath under the CLU_US direcoty
    #     subReadPath = readPath + StateLowerCase + "\\" + "clu\\"                       #this is to get state level read path
    #     unZipShapePath = subReadPath + "clu_public_a_" +  StateLowerCase + CountyFIPS + "_output" #unZipShapePath is to set the unzip file, which is the output path for unzip command
    #     specificUnZipShapePath = unZipShapePath +"\\" + "clu_public_a_" + StateLowerCase + CountyFIPS +".shp"
    #     returnField = 'CALCACRES'
    #     areavalueList = [r[0] for r in arcpy.da.SearchCursor(specificUnZipShapePath, returnField)]
    #
    #     #here is to read the dbf file from here
    #     dbfpath = outputPath + "\\ZonalStUS.dbf"                                #dbfpath is to set the result dbf file that needed to get the computation
    #     #here is some file that can not be read
    #     try:
    #         org_db = dbf.Dbf(dbfpath, new = False)
    #
